...
{
    [Fact]
    public void Execute_GivenRealPipeline_NotReadyBeforeLaunch_RendersDeterministicSummary()
    {
    ...
    public void Execute_GivenReadyStageResults_ComposesToActiveSummary()
    {
        ...
        var originalStartExecutor = GenerateFromCurrentActivateCommand.StartExecutor;
        ...
        try
        {
            ...
            GenerateFromCurrentActivateCommand.StartExecutor = (_, _, _) => new IntakeStartResult
            {
                Domain = "auth",
                StartedExecutionUnits = ["AUTH-01"],
                GeneratedArtifactPaths = [".intent-cli/issues/AUTH-01/packet.yaml"],
                CreatedIssueRefs = ["issue:201"],
                WorktreePaths = [".intent-cli/worktrees/issue-201-auth-01"],
                SkippedUnits = []
            };

            var exitCode = GenerateFromCurrentActivateCommand.Execute(CreateContext("/tmp/intent-system"), ["auth", "--from-path", "src/feature"], writer);

            Assert.Equal(0, exitCode);
            var output = writer.ToString();
            Assert.Contains("Generate-from-current activate processed for domain 'auth'.", output, StringComparison.Ordinal);
            Assert.Contains("- .intent-cli/issues/AUTH-01/packet.yaml", output, StringComparison.Ordinal);
            Assert.Contains("- issue:201", output, StringComparison.Ordinal);
            Assert.Contains("- .intent-cli/worktrees/issue-201-auth-01", output, StringComparison.Ordinal);
        ...
        finally
        {
            ...
            GenerateFromCurrentActivateCommand.StartExecutor = originalStartExecutor;
        }
    }
