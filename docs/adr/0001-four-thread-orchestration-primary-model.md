...
Exactly one mode applies per domain/repo; the two must never run simultaneously for the same domain/repo (unchanged mixed-mode invariant). Loopless-receiver behavior is an **orchestrator-message-mode invariant**: in that mode, implementation and review threads act only when the orchestrator delegates and never start their own recurring timer/loop. In the retained **timer-loop alternative**, implementation and review threads keep their existing, unchanged semantics — they self-schedule on recurring timers and read `worker next-action` / host review-next-slice as their source of truth; they are not loopless receivers in that mode, since no orchestrator thread paces them.
