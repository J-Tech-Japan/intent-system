...
`eng/version.json`**, and see [Next release readiness](#next-release-readiness-v0121)
...
#   expected shape: intent-cli 0.12.1-<sha>-G<unit>   (NOT a stale literal)
...
publish the GitHub Release for `v0.12.1`; publishing it triggers `release.yml`
...
`stableVersion → 0.12.1`, `nextVersion → 0.12.2` — carrying, per **steps 4–6** of the
