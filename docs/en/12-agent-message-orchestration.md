...
`registration-lost-process-present` state and names whether the matching
`agent_session` field is missing; it does not infer process loss. When
`agent_session` is missing while a foreground process remains, the bounded
operator act is one no-op prompt to the already-running seat at the recorded
pane, followed by a fresh readiness/report check. Re-registering, restarting,
killing, or automatically re-registering is prohibited.
...
**Sender-local report routing and executability/actionability diagnostics (G719 — preview-through-1.x).** An implementation seat receives exactly one bounded `--add-dir <role-work-root>` and is not granted `--add-dir <host-routing-root>` or host-root write access. Its canonical report carries `--routing-root <host-routing-root> --report-root .`: the host root remains read/transport authority and the role root is the writable sender-local outbox. A report on a seat that genuinely lacks the host routing root must persist its local outbox, identify `report_storage_mode: sender-local-role-work-root`, and identify `host_state_sync: deferred-to-orchestration`; orchestration reconciles host pending/continuation state with `intent-cli notify reconcile --domain <d> --team <t> --task-id <id> --routing-root <host> --report-root <role-work-root> --write --format json`. It must not retry a host write, hand-write transport, or widen the seat. An outbox entry left `undelivered` is recovered on the host with `intent-cli notify collect --domain <d> --team <t> --task-id <id> --report-root <role-work-root> --routing-root <host> --write --format json`: on a host where the reader file is writable, collect appends the delivery event to the recorded reader, marks the outbox entry delivered, records the host continuation chain, and leaves the pending record for `notify reconcile` to converge. If delivery resolves an external reader under the host root, the seat runs a write probe against that exact reader file; only when the probe is denied does it fail before appending with `report-routing-root-write-required` as a delegation-level routing fault and retain the local handoff, naming `intent-cli notify collect` as the recovery command. The refusal is measured against the write, never inferred from the shape of the roots.
...
The authority boundary, idempotent host reconciliation command, G300/G330
tradeoff, and rejected alternatives are recorded in [ADR 0008 — Sender-local
report reconciliation](../adr/0008-sender-local-report-reconciliation.md).
...
`resend_permitted: true`. If the matching agent-list entry lacks
`agent_session`, the operator sends one no-op prompt to the already-running
seat at the recorded pane to establish it, then reruns the bounded
readiness/report check. No re-registration, restart, kill, or automatic
re-registration is attempted. A genuinely absent process remains `lost`, and
an active registration remains an ownership stop.
