...
**Empirical routing-write refusal (G731 — preview-through-1.x).** When delivery resolves an external reader, the sender attempts the append on that exact reader path and refuses only if the operating system denies the write. `report-routing-root-write-required` is emitted only when the denial was measured, and the message reports the observed error rather than inferring a sandbox from the fact that two roots differ. Because the refusal is empirical, the same command recovers a genuinely `undelivered` entry from a context that can write the reader path: run `intent-cli notify collect --domain <d> --team <t> --task-id <id> --routing-root <host> --report-root <role-work-root> --write --format json` on the host, which appends the retained report and marks the entry delivered. `notify reconcile` only consumes delivered entries and names this exact collect command when it declines an undelivered one.
