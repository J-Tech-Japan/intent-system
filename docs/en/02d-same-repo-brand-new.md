...
Confirm the base-branch policy, transport selection (prefer herdr-only for collocation because it has fewer dependencies; agmsg + herdr is deprecated (G829) but still works for distributed/existing-agmsg teams), and the agent kind for each design, orchestration, implementation, and review role.
