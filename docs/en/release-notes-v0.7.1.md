# Release Notes — intent-cli v0.7.1 (DRAFT — UNRELEASED)

> **⚠️ DRAFT / UNRELEASED.** This is a **stub**, not release notes. It exists
> because `eng/version.json` names `0.7.1` as the release-to-be-cut, and the
> G475 guard requires notes to exist for that version before it can be
> published. **The v0.7.1 release-prep packet authors the real content**;
...
moment `nextVersion` became `0.7.1`. Until the release-prep packet fills it in:

- **No slices are listed yet.** What ships in `v0.7.1` is decided by the
  release-prep packet, not by this stub.
- **No bump rationale yet** (patch vs minor is a release-prep decision).
- **No readiness gate yet.** Do **not** publish a `v0.7.1` GitHub Release while
...
([v0.7.0](release-notes-v0.7.0.md), [v0.6.2](release-notes-v0.6.2.md)):
