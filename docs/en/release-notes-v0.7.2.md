# Release Notes — intent-cli v0.7.2

> **Release model:** a maintainer/operator (or external release automation)
> **creates and publishes the GitHub Release** for `v0.7.2` — the version-bump
...
> and [publishing v0.7.2](#publishing-v071).

## What's in v0.7.2

v0.7.2 covers exactly the five slices merged after `v0.7.1`: **G565**, **G566**,
**G567**, **G568**, and **G569**.
...
The 0.7.2 roll was the first to collide with it. The heading is now written into
...
These items must hold **before the GitHub Release for `v0.7.2` is published**.
...
- [ ] **No DRAFT stub remains in either `release-notes-v0.7.2.md`.** An unfilled
...
- [ ] No open intent-system PR or WIP packet intended for this release is
      ...
      publishing).
- [ ] `eng/version.json` shows `stableVersion` `0.7.0` and `nextVersion` `0.7.2`
...
1. After this packet is merged and the readiness gate above holds, a
   **maintainer/operator (or external release automation) creates and publishes
   the GitHub Release** for `v0.7.2` (tagging the release commit). This is a
...
- [ ] `dotnet tool update -g JTechJapan.IntentSystem.Cli` (or
      `dotnet tool install -g JTechJapan.IntentSystem.Cli --version 0.7.2`)
      then `intent-cli --version` reports `0.7.2`.
- [ ] Binary artifact smoke check: download the platform archive, verify its
      `.sha256`, extract, and run `./intent-cli --version` → `0.7.2`.
...
- [ ] **ROLL `eng/version.json` NOW**, per the G554 rule as amended by G557 and
      completed by G560: `stableVersion → 0.7.2`, `nextVersion → 0.7.2`, in the
