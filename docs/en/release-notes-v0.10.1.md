# Release Notes — intent-cli v0.10.1 (DRAFT — UNRELEASED)

> **⚠️ DRAFT / UNRELEASED.** This is a **stub**, not release notes. It exists
> because `eng/version.json` names `0.10.1` as the release-to-be-cut, and the
> G475 guard requires notes to exist for that version before it can be
> published. **The v0.10.1 release-prep packet authors the real content**;
...
moment `nextVersion` became `0.10.1`. Until the release-prep packet fills it in:

- **No slices are listed yet.** What ships in `v0.10.1` is decided by the
  release-prep packet, not by this stub.
- **No bump rationale yet** (patch vs minor is a release-prep decision).
- **No readiness gate yet.** Do **not** publish a `v0.10.1` GitHub Release while
...
([v0.10.0](release-notes-v0.10.0.md), [v0.7.1](release-notes-v0.7.1.md)):
...
```bash
dotnet tool install -g JTechJapan.IntentSystem.Cli --version 0.10.1
```

Once published, the self-contained binaries will be attached to the
[v0.10.1 GitHub Release](https://github.com/J-Tech-Japan/intent-system/releases/tag/v0.10.1).
