# Release Notes — intent-cli v0.11.2 (DRAFT — UNRELEASED)

> **⚠️ DRAFT / UNRELEASED.** This is a **stub**, not release notes. It exists
> because `eng/version.json` names `0.11.2` as the release-to-be-cut, and the
> G475 guard requires notes to exist for that version before it can be
> published. **The v0.11.2 release-prep packet authors the real content**;
...
moment `nextVersion` became `0.11.2`. Until the release-prep packet fills it in:

- **No slices are listed yet.** What ships in `v0.11.2` is decided by the
  release-prep packet, not by this stub.
- **No bump rationale yet** (patch vs minor is a release-prep decision).
- **No readiness gate yet.** Do **not** publish a `v0.11.2` GitHub Release while
...
([v0.11.1](release-notes-v0.11.1.md), [v0.11.0](release-notes-v0.11.0.md)):
...
```bash
dotnet tool install -g JTechJapan.IntentSystem.Cli --version 0.11.2
```

Once published, the self-contained binaries will be attached to the
[v0.11.2 GitHub Release](https://github.com/J-Tech-Japan/intent-system/releases/tag/v0.11.2).
