...
base-branch policy、トランスポートの選択（同居では依存関係が少ないため herdr-only を優先し、分散 / 既存 agmsg のチームでは非推奨（G829）だが引き続き動作する agmsg + herdr を選ぶ）、design・orchestration・implementation・review 各ロールの agent kind を確認します。
