...
process が消えたとは推測しません。record に `agent_session` がない場合はその欠落を診断値として
表示し、記録済み pane の既存 seat へ 1 回だけ no-op prompt を送り `agent_session` を確立してから
bounded readiness/report check を再実行します。再登録、restart、kill、経過時間だけの verdict 変更は
行いません。
...
message を黙らせず、2 つの outcome を operator が整合できるようにします。
...
**sender-local report routing と executability/actionability 診断 (G719 — preview-through-1.x)。** implementation seat には checkout/worktree 用の境界付き `--add-dir <role-work-root>` を 1 つだけ与え、`--add-dir <host-routing-root>` や MyIntentHost への write access は与えません。canonical report は `--routing-root <host-routing-root> --report-root .` を持ち、host root は read/transport の基準経路、role root は sender-local outbox の write root です。host routing root を実際には書けない seat でも local outbox を保存し、`report_storage_mode: sender-local-role-work-root` と `host_state_sync: deferred-to-orchestration` を示して、orchestration が `intent-cli notify reconcile --domain <d> --team <t> --task-id <id> --routing-root <host> --report-root <role-work-root> --write --format json` で host の pending/continuation state を整合します。host write の retry、hand-write transport、seat boundary の拡大はしません。delivery が host root 配下の external reader を解決する場合は、その正確な reader ファイルに対して write probe を実行し、probe が拒否された場合にのみ append 前に `report-routing-root-write-required` として delegation-level routing fault を返し、local handoff を保持するとともに recovery コマンドとして `intent-cli notify collect` を明示します。拒否は write の実測に基づき、root の形状から推定しません。`undelivered` のまま残った outbox entry は host 上で `intent-cli notify collect --domain <d> --team <t> --task-id <id> --report-root <role-work-root> --routing-root <host> --write --format json` により recovery する。reader ファイルを書き込める host では、collect が delivery event を recorded reader に append し、outbox entry を delivered に更新し、host continuation chain を記録し、pending record は `notify reconcile` の整合に委ねます。

sender-local routing と process-present/missing-registration の 2 つの measured surface は、1 つの executability/actionability class です。state が真でも sandboxed reader が action できないことがあります。`agent_session` の欠落は process death や ownership loss ではありません。active registration は引き続き ownership stop です。

正本境界、idempotent な host reconciliation command、G300/G330 の
tradeoff、rejected alternatives は [ADR 0008 — Sender-local report
reconciliation](../adr/0008-sender-local-report-reconciliation.md) に記録します。
...
返し、delivery は同じ cause と `resend_permitted: true` を返します。record の `agent_session` が
missing なら、recorded pane の既存 seat へ 1 回だけ no-op prompt を送り `agent_session` を確立し、
bounded readiness/report check を再実行します。再登録、kill、restart、automatic re-registration は
行いません。`pane-absent` や
