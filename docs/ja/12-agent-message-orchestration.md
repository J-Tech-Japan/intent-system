...
**empirical routing-write refusal (G731 — preview-through-1.x)。** delivery が external reader を解決する場合、sender はその exact な reader path に対して append を試行し、OS に write を拒否されたときだけ拒否を返す。`report-routing-root-write-required` は拒否を実測したときのみ出力し、メッセージは 2 つの root が異なることからの sandbox 推定ではなく観測された error を報告する。拒否が empirical であるため、reader path を書ける context から同じ command を実行すれば genuinely `undelivered` の entry を recovery できる: host 上で `intent-cli notify collect --domain <d> --team <t> --task-id <id> --routing-root <host> --report-root <role-work-root> --write --format json` を実行すると、保持されていた report が追加され entry が delivered に変わる。`notify reconcile` は delivered の entry のみ消費し、undelivered を拒否する際にはこの collect command を正確に名前として挙げる。
