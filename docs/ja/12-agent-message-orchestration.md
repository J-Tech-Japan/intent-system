...
**sender-local report routing と executability/actionability 診断 (G719 — preview-through-1.x)。** implementation seat には checkout/worktree 用の境界付き `--add-dir <role-work-root>` を 1 つだけ与え、`--add-dir <host-routing-root>` や MyIntentHost への write access は与えません。canonical report は `--routing-root <host-routing-root> --report-root .` を持ち、host root は read/transport の基準経路、role root は sender-local outbox の write root です。host routing root を実際には書けない seat でも local outbox を保存し、`report_storage_mode: sender-local-role-work-root` と `host_state_sync: deferred-to-orchestration` を示して、orchestration が `intent-cli notify reconcile --domain <d> --team <t> --task-id <id> --routing-root <host> --report-root <role-work-root> --write --format json` で host の pending/continuation state を整合します。host write の retry、hand-write transport、seat boundary の拡大はしません。delivery が host root 配下の external reader を解決する場合はその reader path へ実際の append を試み、write 結果が実際に示した事実だけを返します。host root を本当に書けない環境では計測された write 失敗とともに `report-routing-root-write-required` を delegation-level routing fault として返し、local handoff を保持したまま host 側の回収コマンド `intent-cli notify collect --domain <d> --team <t> --task-id <id> --write --routing-root <host> --report-root <role-work-root>` を明示します。reader を書ける host は同じ `collect` で handoff の delivery を一度だけ行い、その後 `notify reconcile` で host の pending/continuation state を閉じられます。
