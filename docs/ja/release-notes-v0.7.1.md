# リリースノート — intent-cli v0.7.1

> **リリースモデル:** `v0.7.1` の **GitHub Release を作成・publish するのは
...
> [v0.7.1 の publish](#v071-の-publish)を参照してください。

## v0.7.1 の内容

v0.7.1 は `v0.7.0` 以降にマージされた 5 スライス — **G565**、**G566**、**G567**、
**G568**、**G569** — を正確にカバーします。

**なぜ minor ではなく patch か。** 文書化されたポリシーは、新しいコマンドサーフェスに
対して minor バンプを予約しています。本リリースにはそれがありません。すべてのスライス
がバグ修正または決定性の修正であり、削除・改名されたコマンド / 引数 / フラグもありま
せん。

明示的に述べておくべきは G568 の `automation queue-dependency-reconcile` です。これは
**バグ修正を完結させる修復ユーティリティ**であって、新しい workflow 能力ではありません。
同じスライスの修正によって今後は生成されなくなる queue item を是正するためだけに存在し、
どの workflow フェーズにも参加せず、自動実行もされず、過去分を reconcile し終えれば
恒常的な役割を持ちません。バグが既に書き込んでしまったデータの修復経路は、そのバグ修正
の一部です。minor の予約は「workflow が新たに*できるようになる*こと」を追加する
サーフェスに対するものであり、これは workflow に何も追加しません。
...
どちらも test-only であり、どちらも同じものを守ります。review と merge のゲートが
canonical として扱う、exact-head の「CI green」というエビデンスです。
...
パスが書き戻していました。0.7.1 の roll がこれに最初に衝突しました。現在は見出しを
...
**内部だけの変更** — CLI 利用者から見える差はありません: G566 と G569 はテストコードの
変更であり、G568 のパーサ配線は、既に正しく seed できていた packet に対して byte 互換
です。

## リリース準備ゲート(G572)

以下は **`v0.7.1` の GitHub Release を publish する前**に満たされている必要があります。
...
- [ ] リリース対象のパケットがすべて**完了し、その PR が `main` にマージ済み**である:
      ...
      host 側の前提条件です。
- [ ] **どちらの `release-notes-v0.7.1.md` にも DRAFT スタブが残っていない。** 未記入の
...
- [ ] `eng/version.json` が `stableVersion` `0.7.0`、`nextVersion` `0.7.1`
...
1. 本パケットがマージされ、上記の準備ゲートが満たされた後、**メンテナ/オペレーター
   (または外部のリリース automation)が `v0.7.1` の GitHub Release を作成・publish
...
- [ ] `dotnet tool update -g JTechJapan.IntentSystem.Cli`(または
      `dotnet tool install -g JTechJapan.IntentSystem.Cli --version 0.7.1`)後、
      `intent-cli --version` が `0.7.1` を報告する。
- [ ] バイナリ成果物のスモークチェック: プラットフォームアーカイブをダウンロードし、
      `.sha256` を検証し、展開して `./intent-cli --version` → `0.7.1`。
...
- [ ] **今すぐ `eng/version.json` を ROLL する**。G554 のルール(G557 による改訂、
      G560 による完成)に従い、`stableVersion → 0.7.1`、`nextVersion → 0.7.2` を、
