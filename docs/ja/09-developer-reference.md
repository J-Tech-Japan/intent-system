...
- **`--packet` モード**では、その packet の `github-body.md` と
  ...
  - **indented code block** — 上記のフェンス認識とは別の、独立した
    ...
    「この行はコードではない」ことは別の問いです。連続する行のうち、
    各行が インデントされている（リテラルなタブ、または 4 個以上の
    リテラルなスペース — 1〜3 スペースは通常の、単に揃えられただけの
    prose であり、意図的にこれだけでは条件を満たしません）か、または
    空行であるものの最大の連続範囲が、1 つの block としてマスクされます。
    範囲の「内部」にある空行はそれを終了させません（indented code
    block の内部にある空行の継続を許容する CommonMark 自身の挙動を
    反映しています）。範囲は、本当に非空・非インデントの行に達した
    時点で必ず終了し、そこから通常の抽出がすぐに再開されます。これは
    SIMPLIFIED な近似です — CommonMark の完全な list-item-continuation
    の判別（インデントを column 0 ではなく list マーカーからの相対位置
    で測る）は再現しません。そのため、list item 内の 4 スペース
