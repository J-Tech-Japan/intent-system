...
- **mode-independent-with-transport-mechanics** セクションは、両モードで拘束する canon を
  ...
  **宣言すること**と**正しく型付けすること**は別です。最初の実装は型を**構成的に**割り当てて
  ...
  agmsg example のラベルは、markdown / JSON いずれでも**それが修飾する記述的フラグメント 1 つに
  対して 1 つ**の粒度で付与します。mixed なセクションやプロパティ全体を「descriptive, not an
  instruction」と宣言すると、読者にも JSON consumer にも「その中の拘束力ある義務は例示である」と
  伝わってしまいます。連続する記述的フラグメントをまとめる banner 方式も誤りでした — 記述と義務は
  交互に現れるため、banner は直後の指示まで視覚的に覆ってしまいます。
