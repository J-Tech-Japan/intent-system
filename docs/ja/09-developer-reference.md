...
**リリース準備検証(`v0.12.0` release-preparation PR のマージ前に実行):**
...
#   期待する形: intent-cli 0.12.0-<sha>-G<unit>   (古いリテラルではない)
