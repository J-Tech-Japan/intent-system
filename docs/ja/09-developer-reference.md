...
**`v0.13.0` は出荷済み**(GitHub Release + NuGet)で、次に準備するラインは
`0.13.1` です。[v0.13.1 notes stub](release-notes-v0.13.1.md) が必須の
prepare-only placeholder です。直前の出荷範囲は
[release-notes-v0.13.0.md](release-notes-v0.13.0.md) を参照してください。

**リリース準備検証(`v0.13.1` release-preparation PR のマージ前に実行):**
...
#   期待する形: intent-cli 0.13.1-<sha>-G<unit>   (古いリテラルではない)
