...
初期化の前に、ホストメタデータをどこに置くかを選択します。2 つのトポロジーが完全にサポートされています。
...
### トポロジー B — メタデータ用ブランチを使う同一リポジトリ

ホストと子実装が **同じリポジトリ** に共存します。ホストメタデータは専用ブランチ（一般的に `main-metadata`）に置き、
実装 PR が実装ベースブランチ（`main`）を対象にしながらメタデータを含まないようにします。
...
- `intent-cli intent init` は **メタデータ用ブランチのチェックアウト** から実行します。
- 実装 PR は `main` を対象にします。メタデータは `main-metadata` に留まります。
- child 実装 agent は両方のトポロジーで **GitHub-contract-only** です。
  メタデータ用ブランチを読んだり変更したりしません。

### どちらのトポロジーを選ぶか

| 考慮点 | 別のホストリポジトリ | 同一リポジトリ + メタデータ用ブランチ |
|---|---|---|
| チームが intent オーケストレーションと実装を明確に分離したい | ✓ 自然な境界 | より多くの規律が必要 |
| 管理するリポジトリ数を減らしたい | リポジトリが増える | ✓ 単一リポジトリ |
| コントリビューターに実装のみ見せたい OSS プロジェクト | ✓ ホストリポジトリを非公開にできる | メタデータ用ブランチは全員に見える |
