...
internal static class TopologyWorkspaceMoveGuidance
{
    ...
    public static TopologyWorkspaceMoveGuide Create(string? domain = null, string? team = null)
    {
        ...
        return new TopologyWorkspaceMoveGuide
        {
            ...
            PaneMapContract = "Supply one old-pane=new-pane pair for every recorded herdr pane. Roles that share one recorded pane travel together to that pane's new pane; mapping two different recorded panes to one new pane stays refused as genuinely ambiguous. New pane ids must belong to the supplied new workspace; external roles have no pane mapping.",
            PreservationContract = "The move changes only the team workspace id and recorded herdr role workspace_id/pane_id values. Role membership, cwd, kind, delivery_method, reader, frontend, launch arguments, profiles, and all other JSON fields remain unchanged. Roles that share one recorded pane keep sharing their new pane: the move performs no pane splits or merges.",
