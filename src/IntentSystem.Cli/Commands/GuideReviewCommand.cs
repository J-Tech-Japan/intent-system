...
internal static class GuideReviewCommand
{
    ...
    internal static readonly IReadOnlyList<string> CrossRuntimeReviewRules = new[]
    {
        ...
        "On both `request` and `record`, `--model` is required for `copilot` and `opencode` and optional for `codex`, `claude`, and `cursor`. `--effort` is supported by `copilot` and `opencode`; request-only `--opencode-provider-config` is supported by `opencode`.",
