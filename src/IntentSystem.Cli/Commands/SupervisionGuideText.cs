...
internal static class SupervisionGuideText
{
    public const string PreviewLabel =
        "Preview through 1.x (G628/G644/G658/G659/G671/G675): supervision setup, scheduler emission, event mode, and explicit pending-delegation dispositions are outside the 1.0 compatibility promise.";
    ...
    public const string InitHostSetup =
        ...
        + DeploymentBasis + " " + RuntimeSelfSufficiency + " " + ReferencePointer + " " + PreviewLabel;

    public static string HostLoopSetup(string domainPlaceholder, string targetRepoPlaceholder) =>
        $"Supervision setup (G644): G658 routes setup through `intent-cli notify supervise install`. Before relying on bounded recovery for `{domainPlaceholder}` / `{targetRepoPlaceholder}`, emit one per-team scheduler artifact for exactly one standing loop outside the agent seats, inspect it, then use the printed registration command as an explicit operator action. Do not put the supervisor inside a seat: it cannot report its own death and dies with that seat. intent-cli neither starts nor manages the background process. {DeploymentBasis} {RuntimeSelfSufficiency} Use `intent-cli guide next --domain {domainPlaceholder} --team <team> --target-repo {targetRepoPlaceholder} --format markdown` to detect a team with no recorded cycle. {ReferencePointer} {PreviewLabel}";

    public static string NextAction(string domain, string team, string repo) =>
        $"Emit the scheduler artifact for exactly one standing supervision loop for `{domain}` / `{team}` outside the seats: `intent-cli notify supervise install --domain {domain} --team {team} --repo {repo} --owner-role <logical-role> --bound <seconds> --interval <seconds> [--event-mode] --write --format json`. Inspect the artifact, then run the exact printed registration command as an explicit operator action; intent-cli executes no registration or process-management command. To adopt event mode, re-emit with `--event-mode` and explicitly re-register because an installed artifact remains interval-only. Verify the live PID and the first cycle record in `cycles.jsonl`, then judge liveness from that record's age against its declared bound, never from process-name grep. {RuntimeSelfSufficiency} {ReferencePointer}";
}
