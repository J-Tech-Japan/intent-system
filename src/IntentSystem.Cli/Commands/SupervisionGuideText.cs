...
internal static class SupervisionGuideText
{
    public const string PreviewLabel =
        "Preview through 1.x (G628/G644/G658/G659/G671): supervision setup, scheduler emission, event mode, and explicit pending-delegation dispositions are outside the 1.0 compatibility promise.";
    ...
    public static string NextAction(string domain, string team, string repo) =>
        $"Emit the scheduler artifact for exactly one standing supervision loop for `{domain}` / `{team}` outside the seats: `intent-cli notify supervise install --domain {domain} --team {team} --repo {repo} --owner-role <logical-role> --bound <seconds> --interval <seconds> [--event-mode] --write --format json`. Inspect the artifact, then run the exact printed registration command as an explicit operator action; intent-cli executes no registration or process-management command. To adopt event mode, re-emit with `--event-mode` and explicitly re-register because an installed artifact remains interval-only. Verify liveness from the age of this team's `cycles.jsonl` record against its declared bound, never from process-name grep. {ReferencePointer}";
}
