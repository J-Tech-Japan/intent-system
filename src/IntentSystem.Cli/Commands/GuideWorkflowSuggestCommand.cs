    ...
    private static GuideWorkflowSuggestion BuildRecommendation(string workflow, string domain, string goalText, IReadOnlyList<string> matchedKeywords, bool includeAdvancedRuntime)
    {
        var (commands, ruleTopics, summary) = workflow switch
        {
            ...
            WorkflowOrchestratorSetup => (
                ...
                "Orchestrator setup — the operator wants to start agmsg orchestrator-message mode. `guide orchestrator-thread` returns a concrete setup INTAKE whose visible outcome is missing-inputs / setup-ready / blocked: pass --domain, --target-repo, --orchestrator-path, --implementation-path, --review-path, --orchestrator-agent, --implementer-agent, --reviewer-agent, --team, --delivery-mode, and --existing-loop-policy to get copy-paste agmsg registration/delivery commands and first role prompts; missing fields are listed instead. The steady state is message-driven — implementation/review replies wake the orchestrator, so routine fast polling is not required — with an explicit orchestrator timer supported only as a fallback/legacy option; implementation/review stay loopless receivers either way. agmsg is a signal layer only — intent-cli and GitHub stay authoritative — and agmsg state is changed only through the agmsg scripts, never by editing its DB/team files."),
