...
internal static class GuideOnboardingCommand
{
    ...
    public static int Execute(CliContext context, string[] args, TextWriter writer)
    {
        ...
        {
            writer.Write(ProgressSupervisionGuidance.AppendJson(CompletionChannelGuidance.AppendJson(JsonSerializer.Serialize(result, JsonOptions))));
            writer.WriteLine();
        }
        else
        {
            ...
            writer.Write(ProgressSupervisionGuidance.AppendMarkdown(CompletionChannelGuidance.AppendMarkdown(markdown.ToString())));
        }
        ...
        return new GuideOnboardingResult
        {
            ...
            FirstCallSequence = new[]
            {
                ...
                new GuideOnboardingStep
                {
                    ...
                    Purpose = "G624: learn which SESSION LAYER this team runs — prefer `herdr-only` for a collocated single-machine team because it has fewer dependencies, or choose deprecated `agmsg` + herdr (still works; removal planned after consumers are checked) for a distributed team or an existing agmsg investment. "
