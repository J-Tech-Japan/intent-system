    ...
    private static readonly IReadOnlyDictionary<string, RulesTopic> Topics = new Dictionary<string, RulesTopic>(StringComparer.Ordinal)
    {
        [TopicLabelOwnership] = new RulesTopic
        {
            ...
            Summary = new[]
            {
                ...
                "When an open issue still carries `intent-pr-created` after its linked PR was closed unmerged and the claim is unheld, use `automation pr-created-stale-recovery` with an explicit ruling instead of manual label removal.",
