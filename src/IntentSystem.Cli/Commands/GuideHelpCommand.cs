...
internal static class GuideHelpCommand
{
    ...
    private static void WriteMarkdown(TextWriter writer)
    {
        ...
        writer.WriteLine("- **Host review / next-slice** — review PRs and plan the next slice: `intent-cli guide review`, `intent-cli review closeout-plan`, `intent-cli review cross-runtime request|record|status` (G834 implementation, G835 `--kind design`, declared teams), `intent-cli automation host-review-preflight`, `intent-cli closeout pr`, `intent-cli issue publish-flow`, and the `guide workflow task review-next-slice-loop` prompt generator.");
