        ...
        return new OrchestratorThreadGuide
        {
            ...
            RoleBoundary = new OrchestratorRoleBoundary
            {
                ...
                MissingPacketResponse =
                    ...
                    + "`intent-cli judgment-wait open --record <design-wait-id> --domain <domain> --team <team> "
                    + "--owner design --blocking-reference <issue|pr|unit|release> --action-needed <judgment-needed> "
                    + "--evidence <facts> --write --format json`. Query it with `intent-cli judgment-wait query "
                    + "--domain <domain> --team <team> --format json`; whoever supplies the judgment MUST resolve it "
                    + "with evidence using `intent-cli judgment-wait resolve --record <design-wait-id> "
            ...
            Scheduling = new OrchestratorScheduling
            {
                ...
                WakeResponsibilities = new[]
                {
                    ...
                    "G764 REQUEST-UPDATE WAKE RULE: applying `intent-pr-request-update` alone does not wake a loopless receiver.",
            ...
            MonitorToolDistinction = new OrchestratorMonitorDistinction
            {
                ...
                FallbackLadder = new[]
                {
                    ...
                    "G673 last-net boundary: if `automation heartbeat` or `automation stalled-work` emits `detection_unavailable=true` / `cause=github-api-quota-exhausted`, do not treat an empty item list as healthy. Record the named resource and `reset_at`, retain and review any `partial=true` local findings, and let orchestration decide whether to wait deliberately; there is no automatic retry, sleep, reset scheduling, or request budgeting. Issue #1442 measured the remote-herdr incident separately; this host's same-day G667 corroboration is a separate attribution.",
                },
            ...
            DesignTrafficController = new OrchestratorDesignTrafficController
            {
                ...
                Playbook = new[]
                {
                    ...
                    "When progress blocks on a design judgment, record that wait before waiting: open judgment-wait with `--owner design`, query the existing record, and whoever supplies the judgment MUST resolve it with evidence. An answered-but-open record is a lie, not a completed design handoff.",
                },
                IdleDiagnostic = new[]
                {
            ...
            WorktreeManagement = new OrchestratorWorktreeManagement
            {
                Summary =
                    ...
                    + "worktree remove`. A sandboxed Codex seat MUST NOT run `git worktree add` under the current recipe: "
                    + "the repository's `.git` root is not declared, and `git worktree add` under the corrected flags is unmeasured; "
                    + "registered worktree creation remains host-state duty. Do not call an arbitrary `/tmp` checkout a managed worktree "
            ...
            MayAnswer = new[]
            {
                ...
                new OrchestratorSupervisionMayAnswer
                {
                    ...
                    Verification =
                        ...
                        + "If it remains, only the exact validated/audited orchestration path may answer it; otherwise escalate.",
                },
                new OrchestratorSupervisionMayAnswer
                {
