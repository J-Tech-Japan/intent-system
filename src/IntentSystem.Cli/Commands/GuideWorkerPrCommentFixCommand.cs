...
- If the REQUESTED EDIT TARGET of every actionable comment is a host metadata path (`.intent-cli/**` or `intents/**`), stop. Set outcome to `host-artifact-repair-required`. These paths are host-owned durable artifacts that child implementation workers must not edit; the host repair agent must handle them. (The `pr-comment-preflight` result will show `classification: host-artifact-repair-required` in this case.) G476: classify by the requested edit target, NOT by any incidental host-path mention — a comment that merely CITES a host path such as `.intent-cli/issues/<unit>/packet.yaml` as packet-aware review evidence (G316) while asking you to change implementation files is `repair-required` / actionable, not host-artifact. The preflight result exposes `actionable_comments[].requested_edit_paths` vs `actionable_comments[].host_evidence_paths` so you can see which paths drove the decision.
