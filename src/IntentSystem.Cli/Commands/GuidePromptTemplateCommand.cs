...
internal static class GuidePromptTemplateCommand
{
    ...
    internal static readonly IReadOnlyList<PromptTemplateSpec> Templates = new[]
    {
        ...
        Template(
            ...
            ["<domain>", "<agent>", "<frequency>", "<cwd>", "<owner/repo>"],
            "intent-cli guide workflow task review-next-slice-loop --domain <domain> --target-repo <owner/repo> --agent <agent> --frequency <frequency> [--implementation-base <branch>] --format markdown",
            "Please ask intent-cli for the detailed host review-and-next-slice conditions, then set up a current-thread host loop for domain `<domain>` using `<agent>` every `<frequency>`. cwd is `<cwd>`. target repo is `<owner/repo>`. Do not create a remote scheduler or read local rules. If the effective implementation/PR base branch is not the default, supply it explicitly via `--implementation-base <branch>` so review validates merges against the same branch child implementation used (e.g. `develop-v2`), not `main`.",
            "intent-cli に詳細条件を確認してから、domain `<domain>` の review-and-next-slice loop を `<agent>` で `<frequency>` 間隔に設定してください。cwd は `<cwd>`、target repo は `<owner/repo>` です。実効 implementation/PR base branch が default でない場合は `--implementation-base <branch>`（例 `develop-v2`）で明示し、review が child implementation と同じ branch に対して検証するようにしてください。"),
        Template(
