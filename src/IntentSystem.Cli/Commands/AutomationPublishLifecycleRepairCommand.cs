...
internal static class AutomationPublishLifecycleRepairCommand
{
    ...
    public static int Execute(CliContext context, string[] args, TextWriter writer)
    {
        ...
        {
            EmitEmpty(writer, format, repo!, write, $"no `.intent-cli/issues/` directory at '{issuesDir}'; nothing to repair.");
            return 0;
        }
    ...
    private static string BuildSummary(PublishLifecycleAnalysis analysis, int appliedCount, int failureCount, bool write)
    {
        if (write)
        {
            return $"publish-lifecycle-repair: applied {appliedCount} repair(s) ({analysis.CoherentCount} coherent, {analysis.DriftCount} drift, {analysis.UnsafeCount} unsafe, {analysis.MissingArtifactCount} missing-artifact, {failureCount} failure(s)).";
        }
        return $"publish-lifecycle-repair (dry-run): {analysis.DriftCount} repairable drift(s); {analysis.CoherentCount} coherent, {analysis.UnsafeCount} unsafe, {analysis.MissingArtifactCount} missing-artifact.";
    }

    private static void EmitEmpty(TextWriter writer, string format, string repo, bool write, string summary)
    {
        var result = new PublishLifecycleRepairResult
        {
            Repo = repo,
    ...
    {
        writer.WriteLine($"# automation publish-lifecycle-repair (G307) — `{result.Repo}` ({result.Mode})");
        writer.WriteLine();
    ...
    private static bool TryParseArguments(string[] args, out string? repo, out bool write, out string format, out string error)
    {
        repo = null;
        write = false;
