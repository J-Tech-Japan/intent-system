...
internal static class HerdrOnlyOperatingGuide
{
    ...
    public static string RenderMarkdown(IReadOnlyList<string> replacedHeadings)
    {
        ...
        The per-role `topology record` workspace mismatch refusal remains correct. Do not hand-edit the machine-local JSON. Run `intent-cli guide topology-workspace-move --domain <domain> --team <team>` to render the installed recipe, then inspect and preview the complete before/after state. The canonical mutation is `intent-cli session-layer topology move --domain <domain> --team <team> --workspace-id <new-workspace-id> --pane-map <old-pane>=<new-pane> [--pane-map ...] --write --format json`; include one mapping for every recorded herdr pane — roles that share one recorded pane travel together to that pane's new pane, and mapping two different recorded panes to one new pane stays refused as genuinely ambiguous — pass `--current-digest <digest>` when carrying a prior preview, and follow it with topology validate and notify `--dry-run` preflight. The move is operator-supplied, CAS-guarded, atomic, dry-run-first, and preserves role membership, cwd, kind, delivery method, readers, profiles, and all other fields. It never queries herdr, discovers a workspace, provisions panes, or repairs a per-role conflict.
