...
internal static class HerdrOnlyOperatingGuide
{
    ...
    public static string RenderMarkdown(IReadOnlyList<string> replacedHeadings)
    {
        ...
        Persist the team-scoped delivery topology at `<host-repo>/.intent-cli/role-pane-mapping.json` only through `intent-cli session-layer topology record --team <team> ... --write`, then run `topology validate --team <team> --format json` and inspect resolved no-send targets with `topology show --team <team> --format json`. Record the team `workspace_id`; under `roles`, give pane-backed roles `resident: herdr` with explicit workspace/pane ids and roles outside herdr `resident: external` with a safe routing-root-relative `reader`. Record uses only operator-supplied values, is idempotent on an exact match, and refuses conflicts; it never queries herdr, guesses ids, provisions, or auto-repairs. Every recorded role can send or be the delegate `report-to`; ONLY the recipient must be deliverable. A herdr recipient requires a running agent at its recorded pane inside that exact team workspace. An external recipient receives one canonical delegate/report event through its recorded reader and returns `eventAppended: true`. `--dry-run` performs the same topology and recipient resolution as `--write`, including team-scoped unknown-role diagnostics, but never prompts or appends. Missing/unsafe readers, foreign-workspace-only agents, and stale mappings fail closed without fallback.
