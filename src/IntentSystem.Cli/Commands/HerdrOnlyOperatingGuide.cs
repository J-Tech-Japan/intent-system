...
internal static class HerdrOnlyOperatingGuide
{
    ...
    public static string RenderMarkdown(IReadOnlyList<string> replacedHeadings)
    {
        ...
        var layout = HerdrStandardLayoutRegistry.RenderMarkdown();
        var dialogRule = DialogAnsweringRuleGuidance.RenderMarkdown();
        return $$"""
        ...
        2. Apply the generated `herdr-standard-layout/v1` registry below for the standard three-pane team shape, exact creation commands, role labels, and measured repair path. Keep the returned workspace/tab/root-pane ids in the operator-visible mapping, resolve explicit ids immediately before every mutation, and use the registry's named check to make layout or label incompleteness visible without turning it into a READY hard block.
        ...
        **Composite success is mandatory:** (1) herdr reports a settled state and pane inspection finds no pending approval/question, (2) the exact `ORCH_RESULT` marker matches the fresh dispatch nonce and status, (3) the named artifact exists and passes the task's verification, and (4) fresh canonical intent-cli/GitHub facts confirm the workflow state. The named artifact verification is the final gate only together with fresh canonical facts. State alone and marker alone NEVER mean task success; in particular, herdr `done` or `idle` alone is insufficient, and a marker found anywhere in the pane buffer is only a signal. `blocked` and `question` markers route back to the orchestrator/design decision boundary; they are not failures to hide or successes to assume.
        ...
        - **Layout repair:** follow the `herdr-standard-layout/v1` measured repair sequence (`herdr pane move --tab --split right|down --target-pane`, then `herdr pane rename`) with explicit ids and operator verification; intent-cli never executes it.
    ...
    public static JsonObject CreateJson(IReadOnlyList<string> replacedProperties) => new()
    {
        ...
        ["layout_registry"] = HerdrStandardLayoutRegistry.CreateJson(),
        ["dialog_answering_rule"] = DialogAnsweringRuleGuidance.CreateJson(),
        ["provisioning"] = new JsonObject
        {
            ...
            ["pane_label"] = "Immediately after recording a herdr pane mapping, run herdr pane rename <pane-id> <logical-role> so the human can identify the pane they are about to supervise; herdr owns this display state and intent-cli never sets it as a topology-record side effect.",
            ["live_validation"] = "Run intent-cli session-layer topology validate --domain <domain> --team <team> --live --format json. The read-only herdr check reports an unlabeled recorded pane as informational and leaves valid true.",
            ...
            ["pane_default"] = $"DEFAULT: consume the herdr-standard-layout/v1 registry for the one-tab three-pane creation sequence, including {HerdrStandardLayoutRegistry.Create().Creation.DefaultPaneSplit}; resolve a non-empty mapped pane id and carry it on every herdr command. The named layout-and-labels check is visible incompleteness only and never a READY hard block.",
            ...
            ["layout_registry_version"] = HerdrStandardLayoutRegistry.RegistryVersion,
            ["setup_check"] = "layout-and-labels: a non-standard layout or role label is visible-incompleteness; ready_blocking=false; the guide never executes herdr.",
        ...
        ["dispatch"] = new JsonObject
        {
            ...
            ["topology"] = "Write machine-local <host-repo>/.intent-cli/topology/<domain>/<team>.json only with intent-cli session-layer topology record --domain <domain> --team <team> ... --write; immediately beside that step, run herdr pane rename <pane-id> <logical-role> so the human can identify the pane they are about to supervise. Herdr owns the display label; intent-cli never calls pane rename or sets labels as a topology-record side effect. The CLI owns a directory-local ignore, records domain/team identity, and re-recording moves machine truth. Use the installed `guide topology-workspace-move` recipe for an intentional workspace rebuild: preview the full before/after, supply every old-pane=new-pane mapping, then use `session-layer topology move ... --write`, validate, and run notify dry-run preflight. A single old pane may carry multiple roles and map to one new pane; only distinct old panes converging on one new pane are refused as ambiguous. session-layer-mode.json remains tracked multi-team truth. Use topology validate/show with the same explicit --domain and --team for aggregate findings and no-send delivery targets; add --live to validate for the read-only herdr pane-label check. Record accepts only operator-supplied values, is idempotent on an exact match, and refuses conflicts without querying herdr, guessing ids, provisioning, or auto-repairing. Each role records resident: herdr plus explicit workspace/pane ids, or resident: external plus a safe routing-root-relative reader.",
            ...
            ["reporting_contract"] = "The generated payload embeds task id, expected artifact, and the complete intent-cli notify report command (including --routing-root <host-routing-root> --report-root . for isolated child checkouts) as the receiver's required final step; the role root owns the sender-local outbox, the host root is read/transport authority, and the implementation seat receives no host-root write grant; never hand-write a transport call.",
        ...
        ["failure_recovery"] = new JsonArray(
            ...
            "Layout repair: use the versioned registry's measured herdr pane move --tab --split right|down --target-pane and herdr pane rename commands with explicit ids; intent-cli never executes the repair.",
