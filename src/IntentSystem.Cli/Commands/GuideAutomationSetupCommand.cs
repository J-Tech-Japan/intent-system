...
3. Stage 2 — next-slice (run when Stage 1 result is `no-actionable-item`, `candidate-ready`, or when Stage 1 review/closeout has cleared the WIP):
   ...
   - If `recommended_outcome` is NOT `issue-cut-ready`: stop with truly `no-actionable-item`. This is the only valid idle stop: Stage 1 found no eligible PR AND Stage 2 found no actionable packet. Queued unpublished packets with satisfied dependencies must be published here rather than ignored.
