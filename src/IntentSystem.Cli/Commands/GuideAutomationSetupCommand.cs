...
2. Stage 1 — review/closeout:
   ...
   - `review-pr` → Review the selected PR: `intent-cli review closeout-plan --pr <n> --repo {targetRepo} --domain {domain} --format json` and `intent-cli guide review --pr <n> --repo {targetRepo} --domain {domain} --format json`. If review passes: run `intent-cli automation pr-transition --transition approved --repo {targetRepo} --pr <n> --write --format json`, then inspect the immutable lane landing_mode. Direct lanes merge via the host's existing merge step and run `intent-cli closeout pr`; operator-merge lanes enter `awaiting-operator-merge`, notify design once, and wait without urge/age escalation until a human merge is detected, then run `intent-cli closeout pr` only. No intent-cli path merges an operator-merge lane. If review needs repair: leave an actionable PR comment, then `intent-cli automation pr-transition --transition request-update --repo {targetRepo} --pr <n> --write --format json`. After Stage 1 closes out or requests repair, proceed to Stage 2 only if the WIP cap is clear.
