...
/// <summary>
/// G204: Read-only <c>intent-cli worker pr-comment-preflight</c> command.
/// Answers "does this GitHub PR currently have actionable review feedback that
/// a worker should repair?" with a single deterministic classification. Never
/// mutates queue state, runs, GitHub labels/PRs/threads/comments, source
/// files, or any on-disk state. Exposes <see cref="PrLookupFactory"/>,
/// <see cref="IssueLookupFactory"/>, <see cref="CommentsLookupFactory"/>, and
/// <see cref="NestedProviderLauncher"/> so tests can inject fakes and assert
/// that no nested provider is launched.
/// </summary>
internal static class WorkerPrCommentPreflightCommand
{
    ...
    /// <summary>
    /// Test seam for the PR comments / reviews / review-threads lookup. Tests
    /// inject a fake <see cref="IGitHubPrCommentsLookup"/> to return canned
    /// payloads. Production callers leave this null and the default
    /// <see cref="GhCliGitHubPrCommentsLookup"/> is used.
    /// </summary>
    ...
    /// that it remains uninvoked across all preflight code paths to lock in
    /// the "no nested provider launch" guarantee.
    /// </summary>
    ...
    public static int Execute(CliContext context, string[] args, TextWriter writer)
    {
        ...
        WorkerPrCommentPreflightResult result;
        try
        {
            result = WorkerPrCommentPreflightAnalyzer.Analyze(
                prPayload,
                commentsPayload,
                repo!,
                prNumber,
        ...
        {
            writer.WriteLine(
                $"failed to classify GitHub PR comments {repo}#{prNumber}: {exception.Message}");
            return 1;
        }
    ...
    {
        writer.WriteLine($"# Worker pr-comment-preflight for {result.Repo}#{result.Pr}");
        writer.WriteLine();
        ...
        writer.WriteLine("## Actionable comments");
        if (result.ActionableComments.Count == 0)
        {
            writer.WriteLine("- (none)");
        }
        else
        {
            foreach (var entry in result.ActionableComments)
            {
                writer.WriteLine(
                    $"- [{entry.Kind}] {entry.Author} ({entry.Id}): {entry.Excerpt}");
            }
        }
        writer.WriteLine();
