...
internal static class GuideWorkflowTaskBugToIntentRepairCommand
{
    ...
    internal static readonly IReadOnlyList<string> Invariants = new[]
    {
        ...
        "Child implementation isolation is preserved: child loops report bugs and STOP; they do NOT plan repair packets, do NOT mutate parent host queue-state / runs.jsonl / packet directories / intent tree / review-runtime state / local rules / local skills. Host metadata gaps are HOST-owned blockers; the metadata-workflow-gap classification routes them to the host loop, never back to the child." + " " + DispatcherSkillCarveOut.BoundaryClause,
