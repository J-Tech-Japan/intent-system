...
internal static class GuidePromptMatrixCommand
{
        ...
        string policyDefaultBranch)
    {
        ...
        // G451: resolve the domain review standing-policy once and surface its
        // source on host-target entries so the host loop's guidance reflects
        // whether it consumed a `.intent-cli/review-policy.json` (domain-file)
        // or the safe built-in defaults. Child entries do not make standing
        // review-policy decisions, so they keep a null source.
        var reviewPolicySource = ReviewStandingPolicyRegistry.Resolve(context, domain).Source;
...
- Never manually apply `intent-target` from the child loop. For issue-to-PR completion, only canonical `worker complete --kind issue --outcome pr-created --github-only --write` may apply target-repository `intent-target`; host-state linkage/publication remains host-owned.
...
{SupervisionGuideText.HostLoopSetup(domainPlaceholder, targetRepoPlaceholder)}
...
5. **Same-repo metadata freshness gate (G362, PR #829 repair)**: `intent-cli automation same-repo-metadata-preflight --format json` — MUST run before any queue-state read (steps 6 / 7 below). Hosts without same-repo topology see `classification: not-configured` and proceed; hosts with `metadata_source_branch` configured must see `clean` before reading queue-state. On `behind-origin` / `missing-local-branch`, apply the recommended `git fetch`/`checkout` and re-run before continuing. On `diverged` / `missing-branch`, stop the wake.
6. `intent-cli intent status --domain {domainPlaceholder} --format json` — current baseline / WIP / queued / clarifications.
7. `intent-cli intent next-slice --dry-run --domain {domainPlaceholder} --target-repo {targetRepoPlaceholder} --format json` — verify WIP cap and clarification gates.

Loop body (single wake):
1. Confirm cwd is the parent host repo root. **Initial pull (G357)**: run `git pull --ff-only origin main` as the VERY FIRST action of every wake — before any intent-cli call, any read-only preflight, and before host-sync-preflight. This guarantees all queue-state, runs.jsonl, packet, and submodule gitlink reads see fresh parent durable state. **Same-repo metadata branch override (G362)**: when `automation summary` reports `same_repo_topology: true` AND a non-empty `metadata_source_branch` (e.g. `main-metadata`), substitute `git pull --ff-only origin <metadata_source_branch>` for EVERY `git pull --ff-only origin main` in this loop body — in same-repo topology the host cwd is checked out on the metadata branch (not on `main`), so pulling `origin main` would fail with non-fast-forward or refresh the wrong source. The `automation summary` output already surfaces both fields; read them once at wake start. If the pull fails (non-fast-forward, dirty worktree conflict, or no upstream), stop and surface the gap to the operator — do NOT proceed past this gate. **Pre-wake host sync (G304)**: AFTER the initial pull, run `intent-cli automation host-sync-preflight --format json`. Exit code 0 (`classification: clean`) is the only state that allows the wake to proceed past this gate. On `classification: behind-origin`, run `git pull --ff-only` and re-run the preflight. On `classification: ff-blocked` (G400 — local `main` has a local-only commit AND `origin/main` advanced, so `git pull --ff-only` is blocked), STOP the wake as a PRE-MUTATION operator stop: report the divergence diagnostics (the preflight's `local_only_commits` and `origin_ahead_commits` short SHA/title, plus `behind_origin_commits` / `ahead_origin_commits`) in stdout ONLY. Do NOT append an abort wake-summary to `.intent-cli/runs.jsonl` and do NOT mutate any durable host-state — `durable_runs_summary_allowed: false` / `pre_mutation_abort: true` mark this; repeated wakes in the same diverged state MUST leave durable state byte-for-byte unchanged. Do NOT reset, discard, rebase, or overwrite the foreign local-only commit(s); the operator must push them to a branch/PR, relocate the work, or explicitly authorize discarding them. Re-run `host-sync-preflight` only after the operator resolves the divergence. On `classification: dirty-host-durable-state`, do NOT immediately abort — run `intent-cli automation durable-state-preflight --domain {domainPlaceholder} --target-repo {targetRepoPlaceholder} --format json` (G312; threading `--target-repo` lets G361's prepared-packet lane refuse a complete-but-wrong-child-repo packet directory instead of silently treating it as commit-ready). When it returns `classification: verified-commit-ready`, the dirty changes are deterministic forward-only metadata updates (e.g. `linked_pr` added to `.intent-cli/queue-state.json`, append-only `.intent-cli/runs.jsonl` events): `git pull --ff-only`, stage ONLY the verified paths returned by the preflight (`git add <path1> <path2>`), commit using the preflight's `recommended_commit_message`, push, then re-run `host-sync-preflight` to confirm `clean` before continuing the wake. On `classification: needs-operator-review` or `unsafe-durable-state`, refuse to continue: surface the dirty paths and the per-path reason to the operator (commit/push if intentional, revert if stale local edits) — do NOT silently stash, overwrite, or auto-commit. On `classification: dirty-mixed`, the durable-state portion still goes through `durable-state-preflight`; the unrelated portion still requires explicit operator handling — refuse to silently mix the two. On `classification: dirty-unrelated-submodule` (G306 safe-stash lane), do NOT abort: run `intent-cli automation workspace-guard --mode begin --write` to stash the unrelated paths into a recoverable safe-stash ref before the wake, then run `--mode end --write` after Stage 1's commit/push lands. If `--mode end` reports a stash-pop conflict, do NOT claim the wake completed; surface the structured recovery instruction. On `classification: submodule-checkout-mismatch` (G357 submodule update lane), do NOT use the safe-stash lane: instead run `git submodule update --init <path>` for each path listed in `submodule_checkout_mismatch_paths`, then re-run `host-sync-preflight` to confirm `classification: clean`. This is safe because the preflight verified the submodule has no local uncommitted changes. After ANY closeout/reconcile/publish-recovery `--write` mutates parent durable state, re-run host-sync-preflight before Stage 2 to confirm the push landed and the working tree is clean again.
2. Stage 1 — review/closeout:
   ...
   - **Draft state is not an operator stop (G398)**: a draft PR selected by `host-review-preflight` is still review-eligible. Draft state ALONE is NOT an operator stop and NOT a reason to skip intent/packet-aware review — review eligibility is distinct from merge eligibility. Run the review now; resolve the draft flag only at the merge boundary (ready-then-approve, request-update, or surface the gap), never by reporting a normal hold solely because the PR is draft.
   - **Draft-aware approval (G297)**: BEFORE applying any approval transition, capture the PR's draft state: `IS_DRAFT=$(gh pr view <n> --repo {targetRepoPlaceholder} --json isDraft --jq .isDraft)`. If `IS_DRAFT == false`, continue with the normal non-draft approval path below. If `IS_DRAFT == true`, do NOT immediately release the lease — that pre-G376 behavior left review-ready drafts looping in repeated lease-release with no outcome. Instead make the draft decision draft-aware (G376): finish gathering the review evidence first (closeout-plan, guide review, base-branch-check, diff check, findings), then run `intent-cli automation host-review-diagnostics --repo {targetRepoPlaceholder} --pr-draft true [--draft-review-ready] [--draft-findings-present] [--operator-intended-draft] --format json`. Pass `--draft-review-ready` ONLY when closeout-plan + guide review are ready, base matches policy, diff check passed, and there are NO implementation findings; pass `--draft-findings-present` when review surfaced implementation findings; pass `--operator-intended-draft` ONLY when the draft state is explicitly operator-intended (never inferred from the draft flag alone). Then handle the returned `classification`: on `draft-ready-to-promote`, mark the PR ready for review (`gh pr ready <n> --repo {targetRepoPlaceholder}`) and CONTINUE the approval/merge/closeout path below — do NOT release the lease; on `draft-request-update`, leave an implementation-actionable comment and run `intent-cli automation pr-transition --transition request-update --repo {targetRepoPlaceholder} --pr <n> --write --format json`; on `draft-merge-blocked` (operator-intended draft, or readiness not verified) do NOT call `pr-transition --transition approved` — run `intent-cli automation pr-transition --transition review-release --repo {targetRepoPlaceholder} --pr <n> --write --format json` to drop `intent-pr-reviewing` cleanly and surface the gap. Never apply final approval while the PR is still a draft; only the promote-then-approve path (after `gh pr ready` clears the draft flag) may proceed to approval.
   - **CI-pending is a defer condition, not a finding (G472)**: at or immediately after candidate discovery, inspect the PR's required checks against its current head SHA — `HEAD_SHA=$(gh pr view <n> --repo {targetRepoPlaceholder} --json headRefOid --jq .headRefOid)` and `gh pr checks <n> --repo {targetRepoPlaceholder}` (or the `statusCheckRollup` for `$HEAD_SHA`). When required checks are still pending/queued/in-progress for the head SHA, DEFER this PR: do NOT acquire `intent-pr-reviewing` solely to wait on CI, and if `review-start` was already applied this wake, release it cleanly via `intent-cli automation pr-transition --transition review-release --repo {targetRepoPlaceholder} --pr <n> --write --format json` rather than leaving a stale review lease. A pending check is NEVER an `implementation-review-finding` and MUST NOT become a `request-update` comment — the implementer cannot repair pending CI from the PR branch. Re-select the PR on a later wake once checks complete; only a concluded FAILED required check is a request-update finding.
   - **Closeout is skill-free (G472)**: review, approval, merge verification, and closeout run through installed intent-cli surfaces only (`review closeout-plan`, `guide review`, `automation pr-transition`, `gh pr view --json merged`, `closeout pr`). Do NOT invoke a local closeout skill (no `/intent-review-closeout`, no copied closeout prompt); historical local skills are convenience adapters, not required dependencies for this loop.
   ...
   - If review passes (and PR is non-draft): the approval summary MUST satisfy `guide review` `approval_summary_requirements` (G316) — name the execution unit + packet contract checked, name the issue Acceptance Criteria verified, state Out-of-Scope was honored, cite at least one related intent/spec/rule reference (or explicitly state none applied), pair the test-pass result with packet/intent evidence, and confirm the PR `Closes #<source-issue>` reference (G311). Do NOT approve when the only evidence in your summary is ""tests passed"". Once the summary is complete, run `intent-cli automation pr-transition --transition approved --repo {targetRepoPlaceholder} --pr <n> --write --format json`, then inspect the immutable lane `landing_mode`. For `direct`, merge via the host's existing merge step, capture `IS_MERGED=$(gh pr view <n> --repo {targetRepoPlaceholder} --json merged --jq .merged)`, and only call `intent-cli closeout pr --pr <n> --repo {targetRepoPlaceholder} --pr-merged $IS_MERGED --write --format json`. For `operator-merge`, approved + green enters visible patient `awaiting-operator-merge`: notify design once, never merge/urge/age-escalate, and wait for a human merge; when GitHub reports merged, resume at `closeout pr --pr-merged true` only. No intent-cli path may merge an operator-merge lane. Stage 2 (next-slice publish) is gated on `closeout pr --write` succeeding; never publish a new child issue while either landing continuation remains open.
   ...
   - **Visible / manual / runtime-gated verification ACs (G383)**: when a PR is blocked ONLY by a visibility/manual/runtime-gated verification acceptance criterion, do NOT ask the operator the same standing A/B/C policy question every wake — the review loop is not the interview loop. Consult `intent-cli guide review-verification-policy --format json` and route deterministically: `standing-policy-approve` (an encoded norm permits approval on source mapping + documented observation with NO false runtime/manual-verification claim) → apply it and continue approval/merge/closeout; `implementation-finding` → leave an actionable PR comment and run `pr-transition --transition request-update` via intent-cli (never ask in chat, never fake the verification); `review-policy-gap` → record the host-owned policy decision ONCE as a durable host clarification/signal (e.g. `intent-cli clarify open` / `clarify record`) and mark it pending so later wakes do not re-ask, and never post a host-policy gap on the child PR as an implementer request. The approval summary MUST state exactly what was verified and what was NOT run — never claim local/Godot/runtime/manual verification that was not executed.
   - **Domain review standing policy (G451)**: `intent-cli guide review --pr <n> --repo {targetRepoPlaceholder} --domain {domainPlaceholder} --format json` now emits a `review_standing_policy` object plus `review_policy_source` (`built-in-default` / `domain-file` / `invalid-fallback-default`), resolved from the OPTIONAL host `.intent-cli/review-policy.json` via the standing-policy registry. CONSUME it for the recurring review decisions instead of re-asking the operator each wake: `draft_handling`, `device_gated_evidence` (apply approve-with-recorded-gap vs hard-block per the configured `approve_with_recorded_gap_allowed` / `hard_block_categories`), `external_artifact_intake`, `test_evidence_sufficiency`, and `follow_up_tracking`. When `review_policy_source: domain-file` the host policy is authoritative — do NOT re-ask a standing-policy question it already answers; when `built-in-default` apply the safe defaults; when `invalid-fallback-default` the file failed to parse — use defaults and surface the warning. This prompt-matrix entry's own `review_policy_source` field reports the resolved source so the host loop can confirm the policy file was consumed.
...
3. Stage 2 — next-slice (only if WIP cap and clarification gates allow). **Post-closeout fresh-state reload (G289, G357)**: when Stage 1 just merged a PR and pushed parent durable state, run `git pull --ff-only origin main` on the host repo BEFORE re-running `intent next-slice --dry-run` — this refreshes `queue-state.json`, `runs.jsonl`, and the submodule gitlink pointer. After the pull, check if any submodule path appears in `host-sync-preflight`'s `submodule_checkout_mismatch_paths`; if so run `git submodule update --init <path>` for each before re-running the preflight. The diagnostic's WIP filter already excludes closed issues / merged PRs (G289 defensive filter), but `intent next-slice --dry-run` reads the local queue-state file and is the authoritative `wip` / `recommended_outcome` source for Stage 2; treat its `wip: []` + `issue-cut-ready` as the green light to proceed even if a stale read of `automation host-review-diagnostics` would have said otherwise.
   ...
   - **Absorbed / superseded / retired packets (G474)**: when `intent next-slice --dry-run` reports `warnings: [""legacy-retirement-marker-needs-machine-metadata""]` (a packet directory carries only a stale human marker such as `STATUS: ABSORBED`), do NOT ask the operator whether to publish it and do NOT publish it. When the absorbed/superseded relationship is deterministic, retire it through intent-cli: `intent-cli packet retire --execution-unit <unit> --absorbed-by <unit> --reason ""<text>"" --write` (or `--superseded-by <unit>` / `--retired`). This records machine-readable lifecycle metadata, preserves the packet files as design history, and excludes the packet from future `issue-cut-ready` selection. Never delete the packet directory merely to avoid publication.
...
Hard rules:
{RenderLoopWakeAskingPolicy()}
- **Continuation contract (G454/G678)**: workflow labels are state markers, not completion boundaries. `intent-pr-approved` is INTERMEDIATE — inspect the immutable lane landing mode before landing. A direct lane continues in the SAME wake through merge, `merged == true`, and `closeout pr --write` unless a concrete gate blocks it. An operator-merge lane enters `awaiting-operator-merge` after approved + green, sends one design notification, and ends the wake in patient human-owned wait; it is never urged or age-escalated, and resumes at closeout only after a human merge is detected. A wake that stops at `intent-pr-approved` without selecting one of those continuations is INCOMPLETE. For a recoverable blocker, repair and retry ONCE; never loop or bypass landing authority.
...
- **Duplicate publish is fail-closed (G481)**: when a single execution unit resolves to MORE THAN ONE GitHub issue (concurrent or duplicate host publish), run `intent-cli automation state-doctor --repo {targetRepoPlaceholder} --format json` and read its classification before touching any state. `concurrent-host-publish-detected`, `canonical-issue-mismatch`, and `pr-closes-noncanonical-issue` are fail-closed: STOP with the classification and exactly one operator action — do NOT pick a canonical issue by recency, do NOT continue both competing publishers, do NOT arbitrarily reopen/close issues, and do NOT auto-edit a PR body during the race. Canonical selection prefers durable evidence (queue-state `linked_issue`, then packet `publish.yaml`, then runs.jsonl `issue-created`) over live GitHub recency. A safe repair (closing the non-canonical duplicate) is offered ONLY as `duplicate-execution-unit-issue-detected` when the canonical issue is unique from durable evidence AND the non-canonical duplicate carries no active PR.
...
- Process at most one PR review and one new child issue per wake.";
        ...
        return new GuidePromptMatrixEntry
        {
            ...
            [
                ...
                "intent-cli guide commands list --format json",
                $"intent-cli automation summary --domain {domainPlaceholder} --format json",
                "intent-cli automation same-repo-metadata-preflight --format json",
...
Loop body (single wake only — do not repeat):
1. Confirm cwd is the parent host repo root. **Initial pull (G357)**: run `git pull --ff-only origin main` as the VERY FIRST action — before any intent-cli call, any read-only preflight, and before host-sync-preflight. This guarantees all queue-state, runs.jsonl, packet, and submodule gitlink reads see fresh parent durable state. **Same-repo metadata branch override (G362)**: when `automation summary` reports `same_repo_topology: true` AND a non-empty `metadata_source_branch` (e.g. `main-metadata`), substitute `git pull --ff-only origin <metadata_source_branch>` for EVERY `git pull --ff-only origin main` in this one-shot body — in same-repo topology the host cwd is checked out on the metadata branch (not on `main`), so pulling `origin main` would fail with non-fast-forward or refresh the wrong source. If the pull fails (non-fast-forward, dirty worktree conflict, or no upstream), stop and surface the gap to the operator — do NOT proceed past this gate. **Pre-wake host sync (G304)**: AFTER the initial pull, run `intent-cli automation host-sync-preflight --format json`. Exit code 0 (`classification: clean`) is the only state that allows the wake to proceed past this gate. On `classification: behind-origin`, run `git pull --ff-only` and re-run the preflight. On `classification: ff-blocked` (G400 — local `main` has a local-only commit AND `origin/main` advanced, so `git pull --ff-only` is blocked), STOP the wake as a PRE-MUTATION operator stop: report the divergence diagnostics (the preflight's `local_only_commits` and `origin_ahead_commits` short SHA/title, plus `behind_origin_commits` / `ahead_origin_commits`) in stdout ONLY. Do NOT append an abort wake-summary to `.intent-cli/runs.jsonl` and do NOT mutate any durable host-state — `durable_runs_summary_allowed: false` / `pre_mutation_abort: true` mark this; repeated wakes in the same diverged state MUST leave durable state byte-for-byte unchanged. Do NOT reset, discard, rebase, or overwrite the foreign local-only commit(s); the operator must push them to a branch/PR, relocate the work, or explicitly authorize discarding them. Re-run `host-sync-preflight` only after the operator resolves the divergence. On `classification: dirty-host-durable-state`, do NOT immediately abort — run `intent-cli automation durable-state-preflight --domain {domainPlaceholder} --target-repo {targetRepoPlaceholder} --format json` (G312; threading `--target-repo` lets G361's prepared-packet lane refuse a complete-but-wrong-child-repo packet directory instead of silently treating it as commit-ready). When it returns `classification: verified-commit-ready`, the dirty changes are deterministic forward-only metadata updates (e.g. `linked_pr` added to `.intent-cli/queue-state.json`, append-only `.intent-cli/runs.jsonl` events): `git pull --ff-only`, stage ONLY the verified paths returned by the preflight (`git add <path1> <path2>`), commit using the preflight's `recommended_commit_message`, push, then re-run `host-sync-preflight` to confirm `clean` before continuing the wake. On `classification: needs-operator-review` or `unsafe-durable-state`, refuse to continue: surface the dirty paths and the per-path reason to the operator (commit/push if intentional, revert if stale local edits) — do NOT silently stash, overwrite, or auto-commit. On `classification: dirty-mixed`, the durable-state portion still goes through `durable-state-preflight`; the unrelated portion still requires explicit operator handling — refuse to silently mix the two. On `classification: dirty-unrelated-submodule` (G306 safe-stash lane), do NOT abort: run `intent-cli automation workspace-guard --mode begin --write` to stash the unrelated paths into a recoverable safe-stash ref before the wake, then run `--mode end --write` after Stage 1's commit/push lands. If `--mode end` reports a stash-pop conflict, do NOT claim the wake completed; surface the structured recovery instruction. On `classification: submodule-checkout-mismatch` (G357 submodule update lane), do NOT use the safe-stash lane: instead run `git submodule update --init <path>` for each path listed in `submodule_checkout_mismatch_paths`, then re-run `host-sync-preflight` to confirm `classification: clean`. After ANY closeout/reconcile/publish-recovery `--write` mutates parent durable state, re-run host-sync-preflight before Stage 2 to confirm the push landed and the working tree is clean again. **Same-repo metadata branch freshness (G362)**: AFTER host-sync-preflight returns `clean` AND BEFORE running `intent status` / `intent next-slice` / any queue-state read, run `intent-cli automation same-repo-metadata-preflight --format json`. The command resolves the configured `metadata_source_branch` (G362 field, falls back to `metadata_branch`) and reports `clean` / `behind-origin` / `diverged` / `not-configured` / `missing-branch` / `missing-local-branch`. `clean` is the only state that allows the wake to proceed past this gate; `not-configured` means the host has not opted into same-repo gates (legacy G357 pull-first main behavior already covers the source). On `behind-origin`, run the recommended `git fetch origin <branch> && git checkout <branch> && git pull --ff-only origin <branch>` then re-run the preflight. On `missing-local-branch` (PR #829 repair — fresh-checkout bootstrap where `origin/<branch>` is fetched but no local ref exists), run `git fetch origin <branch> && git checkout -t origin/<branch>` to create/track the local branch then re-run the preflight — this is NOT a fork. On `diverged` or `missing-branch`, stop with structured operator stop — do NOT read queue-state from a stale or forked branch.
2. Stage 1 — review/closeout:
   - `intent-cli automation host-review-preflight --repo {targetRepoPlaceholder} --format json` to find an eligible PR.
   - **Draft state is not an operator stop (G398)**: a draft PR selected by `host-review-preflight` is still review-eligible. Draft state ALONE is NOT an operator stop and NOT a reason to skip intent/packet-aware review — review eligibility is distinct from merge eligibility. Run the review now; resolve the draft flag only at the merge boundary (ready-then-approve, request-update, or surface the gap), never by reporting a normal hold solely because the PR is draft.
   - **Draft-aware approval (G297)**: BEFORE applying any approval transition, capture the PR's draft state: `IS_DRAFT=$(gh pr view <n> --repo {targetRepoPlaceholder} --json isDraft --jq .isDraft)`. If `IS_DRAFT == false`, continue with the normal non-draft approval path below. If `IS_DRAFT == true`, do NOT immediately release the lease — that pre-G376 behavior left review-ready drafts looping in repeated lease-release with no outcome. Instead make the draft decision draft-aware (G376): finish gathering the review evidence first (closeout-plan, guide review, base-branch-check, diff check, findings), then run `intent-cli automation host-review-diagnostics --repo {targetRepoPlaceholder} --pr-draft true [--draft-review-ready] [--draft-findings-present] [--operator-intended-draft] --format json`. Pass `--draft-review-ready` ONLY when closeout-plan + guide review are ready, base matches policy, diff check passed, and there are NO implementation findings; pass `--draft-findings-present` when review surfaced implementation findings; pass `--operator-intended-draft` ONLY when the draft state is explicitly operator-intended (never inferred from the draft flag alone). Then handle the returned `classification`: on `draft-ready-to-promote`, mark the PR ready for review (`gh pr ready <n> --repo {targetRepoPlaceholder}`) and CONTINUE the approval/merge/closeout path below — do NOT release the lease; on `draft-request-update`, leave an implementation-actionable comment and run `intent-cli automation pr-transition --transition request-update --repo {targetRepoPlaceholder} --pr <n> --write --format json`; on `draft-merge-blocked` (operator-intended draft, or readiness not verified) do NOT call `pr-transition --transition approved` — run `intent-cli automation pr-transition --transition review-release --repo {targetRepoPlaceholder} --pr <n> --write --format json` to drop `intent-pr-reviewing` cleanly and surface the gap. Never apply final approval while the PR is still a draft; only the promote-then-approve path (after `gh pr ready` clears the draft flag) may proceed to approval.
...
internal sealed record GuidePromptMatrixEntry
{
    ...
    /// <summary>
    /// G451: the resolved domain review standing-policy source for host-target
    /// entries — <c>domain-file</c> when a host
    /// <c>.intent-cli/review-policy.json</c> is present, otherwise
    /// <c>built-in-default</c> (or <c>invalid-fallback-default</c>). null for
    /// child-target entries, which do not make standing review-policy decisions.
    /// Lets a host loop see that its guidance consumed the domain policy file.
    /// </summary>
