...
1. Confirm cwd is the parent host repo root. **Pre-wake host sync (G304)**: BEFORE any read-only preflight, run `intent-cli automation host-sync-preflight --format json`. Exit code 0 (`classification: clean`) is the only state that allows the wake to proceed past this gate. On `classification: behind-origin`, run `git pull --ff-only` and re-run the preflight. On `classification: dirty-host-durable-state` or `dirty-mixed`, refuse to continue: surface the dirty paths to the operator (commit/push if intentional, revert if stale local edits) — do NOT silently stash, overwrite, or commit unrelated changes from the wake. On `classification: dirty-unrelated-submodule`, surface separately so the operator can reconcile without touching durable host-state. After ANY closeout/reconcile/publish-recovery `--write` mutates parent durable state, re-run host-sync-preflight before Stage 2 to confirm the push landed and the working tree is clean again.
