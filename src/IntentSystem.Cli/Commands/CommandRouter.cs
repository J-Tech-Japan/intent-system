...
internal static class CommandRouter
{
    ...
    internal static readonly IReadOnlyDictionary<string, string> GroupHelpHints =
        new Dictionary<string, string>(StringComparer.Ordinal)
        {
            ...
            ["notify"] = "`intent-cli notify delegate|report|escalate|status|supervise --domain <d> --team <t> ...`; use `intent-cli notify supervise install ...` to emit an operator-managed scheduler artifact without registration or process execution (canonical role workflow, pending-delegation query, and preview recovery supervisor; transport follows the recorded session-layer mode).",
