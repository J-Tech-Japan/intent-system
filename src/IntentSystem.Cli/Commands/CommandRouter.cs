...
internal static class CommandRouter
{
    ...
    [
        ...
        // former operator-attention name remains a 1.x compatibility alias.
        "judgment-wait",
        "operator-attention"
    ];
    ...
    internal static readonly IReadOnlyList<string> AutomationCommandHelp =
    [
        ...
        "automation branch-lane-confirm-record --execution-unit <u> --actor <actor> --evidence <text> [--domain <d>] [--team <t>] [--actor-role operator|orchestration] [--write]",
        "automation pr-created-stale-recovery --repo <r> --issue <n> --execution-unit <unit> --team <team> --ruling <text> [--write] [--format json|markdown]",
    ...
    private static readonly IReadOnlyDictionary<string, IReadOnlyDictionary<string, CommandHandler>> ImplementedCommands =
        new Dictionary<string, IReadOnlyDictionary<string, CommandHandler>>(StringComparer.Ordinal)
        {
            ...
            ["session-layer"] = new Dictionary<string, CommandHandler>(StringComparer.Ordinal)
            {
                ...
                ["inspect"] = SessionLayerInspectCommand.Execute,
            },
            ...
            ["notify"] = new Dictionary<string, CommandHandler>(StringComparer.Ordinal)
            {
                ...
                ["reconcile"] = NotifyCommand.ExecuteReconcile,
                ["acknowledge"] = NotifyCommand.ExecuteAcknowledge,
                ["ack"] = NotifyCommand.ExecuteAcknowledge,
            ...
            ["claim"] = new Dictionary<string, CommandHandler>(StringComparer.Ordinal)
            {
                ...
                ["stranded"] = ClaimCommand.ExecuteStranded,
            },
            [OperatorAttentionCommand.CanonicalCommandGroup] = new Dictionary<string, CommandHandler>(StringComparer.Ordinal)
            {
            ...
            [OperatorAttentionCommand.DeprecatedAliasCommandGroup] = new Dictionary<string, CommandHandler>(StringComparer.Ordinal)
            {
                ["open"] = OperatorAttentionCommand.ExecuteDeprecatedAliasOpen,
                ["resolve"] = OperatorAttentionCommand.ExecuteDeprecatedAliasResolve,
                ["supersede"] = OperatorAttentionCommand.ExecuteDeprecatedAliasSupersede,
                ["query"] = OperatorAttentionCommand.ExecuteDeprecatedAliasQuery
            },
            ...
            ["automation"] = new Dictionary<string, CommandHandler>(StringComparer.Ordinal)
            {
                ...
                ["continuation-chain"] = AutomationContinuationChainCommand.Execute,
                ...
                ["pr-created-stale-recovery"] = AutomationPrCreatedStaleRecoveryCommand.Execute,
            ...
            ["guide"] = new Dictionary<string, CommandHandler>(StringComparer.Ordinal)
            {
                ...
                // G697: installed topology workspace-move recipe.
                ["topology-workspace-move"] = GuideTopologyWorkspaceMoveCommand.Execute,
                ...
                // G833: solo conductor per-unit contract; render-only and
                // metadata-free like the Steward guide.
                ["solo-conductor"] = GuideSoloConductorCommand.Execute,
    ...
    internal static readonly IReadOnlyDictionary<string, string> GroupHelpHints =
        new Dictionary<string, string>(StringComparer.Ordinal)
        {
            ...
            ["claim"] = "`intent-cli claim acquire --scope execution-unit:<EU> --actor <actor> --team <team> --write`, then `intent-cli claim verify --scope <scope> --team <team>` before a start surface; use `intent-cli claim stranded --format json` to report configured metadata-branch records absent from canonical and `claim stranded migrate ... --confirm-migrate-stranded --dry-run|--write` for explicit transaction-backed migration; release/takeover are explicit attributed pushes and age never changes ownership.",
    ...
    internal static readonly IReadOnlyList<string> WorkflowGuidePointersHelp = new[]
    {
        ...
        "next (design-side action advisor) — `intent-cli next --domain <d> --team <team> --target-repo <r> --format markdown` (alias of `intent-cli guide next`): recommends ONE design-side process, `supervision-setup` when a team declared in `[supervision] opt_in_teams` has no recorded cycle (G828: supervision is opt-in), and realignment when no durable improve run falls within the independently declared window, G465/G644/G662. Read-only and recency-only; never grades quality, auto-executes, or manages a process.",
