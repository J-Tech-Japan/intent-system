    ...
    internal static SoloConductorBuilder BuildBuilder() => new()
    {
        ...
        [
            ...
            new SoloConductorBuilderInvocation
            {
                ...
                Enforcement = "a file-tool write outside the clone and `git push` refused; an outside shell write not refused.",
                Measured = true,
            },
            new SoloConductorBuilderInvocation
            {
            ...
            new SoloConductorBuilderInvocation
            {
                ...
                Enforcement = ModelRequiredPrefix
                    + "an inside write and a commit succeeded; `git push` refused, even through `task` and with a permissive clone and global config; no plugin or MCP server ran; outside writes unmeasured here.",
                Measured = true,
            },
        ],
    ...
    internal static SoloConductorGuide BuildGuide() => new()
    {
        ...
        [
            ...
            new SoloConductorLoopStep
            {
                ...
                Instruction = "Author the packet, validate the body, run the facet check, and dry-run publication. Read `title_source` and every warning before writing. For a team declared in `[[cross_runtime_review.teams]]` (G835), after the dry runs render one `review cross-runtime request --kind design --execution-unit <unit> --runtime <runtime> --out-dir <out-dir> [--model <model>] [--effort <level>] [--opencode-provider-config <file>]` for the declared conductor runtime and one for a different runtime; the seat runs each invocation; `review cross-runtime record --kind design --execution-unit <unit> --packet-digest <sha256> --runtime <runtime> --runtime-version <version> --verdict-file <verdict-file> [--model <model>] [--effort <level>] --write`; `review cross-runtime status --kind design`; after any packet edit the digest changes and the review repeats. On both `request` and `record`, `--model` is required for `copilot` and `opencode` and optional for `codex`, `claude`, and `cursor`; `--effort` is supported by `copilot` and `opencode`; request-only `--opencode-provider-config` is supported by `opencode`. Records copied from another host clone with a skewed clock can empty the epoch and fail closed. Dry-run `issue publish-flow` reports the design gate even when the issue exists only on GitHub. Only `issue publish-flow` is gated; `issue draft` / `issue create`, `issue publish-reviewed`, `queue dispatch`, and `bug implementation-issue` bypass the design gate and are not part of this loop.",
                Commands =
                [
                    ...
                    IntentCli("intent-cli review cross-runtime record --kind design --execution-unit <unit> --packet-digest <sha256> --runtime <runtime> --runtime-version <version> --verdict-file <verdict-file> [--model <model>] [--effort <level>] --write --format json"),
            ...
            new SoloConductorLoopStep
            {
                ...
                [
                    ...
                    IntentCli("intent-cli review cross-runtime record --repo <owner/repo> --pr <pr> --head-sha <head-sha> --execution-unit <unit> --kind implementation --runtime <runtime> --runtime-version <version> --verdict-file <verdict-file> [--model <model>] [--effort <level>] --comment-out <review-body> --write --format json"),
