...
internal static class GuideCommandsListCommand
{
    ...
    internal static readonly IReadOnlyList<CommandGroupEntry> Groups = new[]
    {
        ...
        new CommandGroupEntry
        {
            ...
            Mutability = MutabilityMixed,
            RecommendedCaller = CallerChatAgent,
            Purpose = "Transport-neutral role workflow (G578/G588/G629/G630/G658/G659/G671/G704/G712): `intent-cli notify delegate|report|escalate|dispose|status|supervise`; `notify dispose` explicitly records a disposition for an open pending delegation, while late reports remain deliverable. Optional `notify supervise --event-mode` keeps blocking herdr waits inside the same supervisor process while interval cycles remain the safety floor; event-mode adoption still requires re-emission and explicit current-session registration because the artifact embeds the invocation. `notify supervise install` rejects a bound below the interval, emits an observable artifact with routing-root/log paths, and reports success only after bounded first-cycle proof; it prints current-session registration/unregistration commands without executing install lifecycle commands. `notify supervise reconcile|uninstall --write` reports loaded-before/after, unloads managed jobs, and removes managed plus legacy login-persistent artifacts. Duplicate writer findings use G699 backoff/park and never auto-kill. The CLI resolves the recorded session-layer mode internally, validates roles in the requested team topology, records pending delegations durably before delivery, and exposes preview status and bounded recovery supervision. In herdr-only mode pane residents receive at their recorded team-workspace pane while external residents receive delegate/report events through their recorded reader. Dry-run performs route, supervision, and scheduler-artifact resolution without prompting, replacing, redispatching, appending, writing, or executing; delivery, pending records, recovery mutations, dispositions, artifact writes, and lifecycle reconciliation require `--write`."
        },
        new CommandGroupEntry
        {
            Name = "prompt-class",
