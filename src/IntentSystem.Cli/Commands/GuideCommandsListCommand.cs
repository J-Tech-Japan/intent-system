...
internal static class GuideCommandsListCommand
{
    ...
    internal static readonly IReadOnlyList<CommandGroupEntry> Groups = new[]
    {
        ...
        new CommandGroupEntry
        {
            ...
            Mutability = MutabilityMixed,
            RecommendedCaller = CallerOperator,
            Purpose = "Session-layer transport, delivery topology, and host-local launch measurement (G570/G592/G604/G624/G685): `intent-cli session-layer show` reads the team transport and `session-layer set --mode agmsg|herdr-only` records the choice — preferred `herdr-only` has fewer dependencies for a collocated single-machine team; supported, non-retired `agmsg` + herdr remains for a distributed team or existing agmsg investment. The PRIMARY four-thread model is unchanged in either transport; no transport is primary. `session-layer topology record|show|validate --domain <d> --team <t>` canonically writes and checks machine-local `.intent-cli/topology/<domain>/<team>.json` with directory-local CLI-owned ignore and internal identity. `session-layer model-resolution record|query` appends or reads host-local verified/refused model invocation evidence; it launches no provider, validates no id, and ships no model catalogue. Topology record is dry-run by default, idempotent on an exact match, and refuses conflicts; show/validate are read-only and never query herdr or send."
        },
        new CommandGroupEntry
        {
            Name = "notify",
            ...
            Mutability = MutabilityMixed,
            RecommendedCaller = CallerChatAgent,
            Purpose = "Transport-neutral role workflow (G578/G588/G629/G630/G658/G659/G671): `intent-cli notify delegate|report|escalate|dispose|status|supervise`; `notify dispose` explicitly records a disposition for an open pending delegation, while late reports remain deliverable. Optional `notify supervise --event-mode` keeps blocking herdr waits inside the same supervisor process while interval cycles remain the safety floor. `notify supervise install` emits an operator-managed launchd, Task Scheduler, or systemd artifact and exact registration/unregistration commands without executing them; event-mode adoption requires re-emission and explicit re-registration because artifacts embed the invocation. The CLI resolves the recorded session-layer mode internally, validates roles in the requested team topology, records pending delegations durably before delivery, and exposes preview status and bounded recovery supervision. In herdr-only mode pane residents receive at their recorded team-workspace pane while external residents receive delegate/report events through their recorded reader. Dry-run performs route, supervision, and scheduler-artifact resolution without prompting, replacing, redispatching, appending, writing, or executing; delivery, pending records, recovery mutations, dispositions, and artifact writes require `--write`."
        },
        new CommandGroupEntry
        {
            Name = "prompt-class",
