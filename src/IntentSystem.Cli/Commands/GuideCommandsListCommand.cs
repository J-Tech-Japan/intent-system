...
internal static class GuideCommandsListCommand
{
    ...
    internal static readonly IReadOnlyList<CommandGroupEntry> Groups = new[]
    {
        ...
        new CommandGroupEntry
        {
            ...
            Mutability = MutabilityMixed,
            RecommendedCaller = CallerChatAgent,
           Purpose = "Transport-neutral role workflow (G578/G588/G629/G630/G658): `intent-cli notify delegate|report|escalate|status|supervise`; `notify supervise install` emits an operator-managed launchd, Task Scheduler, or systemd artifact and exact registration/unregistration commands without executing them. The CLI resolves the recorded session-layer mode internally, validates roles in the requested team topology, records pending delegations durably before delivery, and exposes preview status and bounded recovery supervision. In herdr-only mode pane residents receive at their recorded team-workspace pane while external residents receive delegate/report events through their recorded reader. Dry-run performs route, supervision, and scheduler-artifact resolution without prompting, replacing, redispatching, appending, writing, or executing; delivery, pending records, recovery mutations, and artifact writes require `--write`."
        },
        new CommandGroupEntry
        {
            Name = "automation",
