...
internal static class SessionLayerFragments
{
    ...
    [
        ...
        Fragment(S1, Operative("- `intent-cli notify supervise install --domain __DOMAIN__ --team __TEAM__ --repo __OWNER__/__REPO__ --owner-role orchestration --bound 900 --interval 300 --startup-bound 120 --write --format json`")),
        Fragment(S1, Operative("- `intent-cli notify supervise install --domain __DOMAIN__ --team <team> --repo __OWNER__/__REPO__ --owner-role orchestration --bound 900 --interval 300 --startup-bound 120 --write --format json`")),
        ...
        Fragment(S1, Operative("- G781 startup proof: `--startup-bound <seconds>` defaults to 120s, reads at most once per second, and must observe a writer-bearing first cycle before a write reports success")),
        ...
        Fragment(S1, Operative("- **install** — `intent-cli notify supervise install --domain __DOMAIN__ --team __TEAM__ --repo __OWNER__/__REPO__ --owner-role orchestration --bound 900 --interval 300 --startup-bound 120 --platform macos --write --format json`")),
        Fragment(S1, Operative("- **install** — `intent-cli notify supervise install --domain __DOMAIN__ --team <team> --repo __OWNER__/__REPO__ --owner-role orchestration --bound 900 --interval 300 --startup-bound 120 --platform macos --write --format json`")),
        ...
        Fragment(S1, Operative("- G781 first-cycle proof failure is named `first-cycle-proof-failed`: `no-post-install-process` names registration without a missing log path, while `post-install-process-wrote-no-cycle` names only existing logs; `install --verify` proves a late start")),
        ...
        Fragment(
            ...
            Scaffold(" "),
            Operative("The coordination responsibility is to route one bounded publish request to the recorded host-state role."),
            Scaffold(" "),
        ...
        Fragment(S5, Operative("- Authorized host-state role executes: intent-cli issue publish-flow <execution-unit> --repo __OWNER__/__REPO__ --write --format json")),
        Fragment(S5, Operative("- Authorized host-state role executes: intent-cli automation issue-publish --write --format json")),
        Fragment(
            S5,
            Operative("- The orchestrator routes and verifies; a sandboxed Codex seat never executes these write-bearing commands itself."),
            Scaffold(" "),
            Operative("Never raw `gh issue create` or `gh ... --add-label`; publication and the `intent-target` label go through the canonical intent-cli surfaces only.")),
        Fragment(S5, Operative("- Confirm via intent-cli / GitHub (not chat) that the host-state role's result is terminal, the issue exists with the expected execution-unit body, and the `intent-target` label is present.")),
        Fragment(S5, Operative("- Confirm the durable workflow state (queue-state / linkage / label) reflects the host-state role's publish through intent-cli surfaces.")),
        Fragment(
            S5,
        ...
        Fragment(
            ...
            Scaffold(" "),
            Operative("a topology-declared design host-state role is legitimate and is routed as a recorded duty."),
            Scaffold(" "),
            Operative("The declaration does not supply a non-sandboxed participant or create host capability."),
            Scaffold(" "),
            Operative("At most one issue per wake; verify the host-state result before delegating implementation.")),
        Fragment(
            S7,
            Operative("- **escalation boundary** — Routine delegation (host-state publish routing, delegate, CI wait, review, closeout) stays orchestrator↔receivers/host-state role and does NOT go to design."),
            Scaffold(" "),
        ...
        Fragment(
            S9,
            Descriptive("Orchestrated work creates temporary workspaces for implementation and review."),
            Scaffold(" "),
            Operative("A non-sandboxed host-state role registers managed worktrees under the workspace and cleans them up with `git worktree remove`."),
            Scaffold(" "),
            Operative("A sandboxed Codex seat MUST NOT run `git worktree add` under the current recipe: the repository's `.git` root is not declared, and `git worktree add` under the corrected flags is unmeasured; registered worktree creation remains host-state duty."),
            Scaffold(" "),
            Operative("Do not call an arbitrary `/tmp` checkout a managed worktree or delete it with raw `rm -rf`; when the delegation explicitly supplies a unit-scoped role-work-root (for example `/private/tmp/<role>-<unit>`), that is an ordinary temporary checkout and the host-state role owns its cleanup."),
            Scaffold(" "),
            Descriptive("Safe routing, not disabling approvals, is the right default.")),
        Fragment(
            S9,
            Operative("- **managed root** — The non-sandboxed host-state role allocates registered worktrees under a repo/workspace-scoped managed root — the `[project] worktree_root` (default `.intent-cli/worktrees/`) — and the exact managed root MUST be listed in the target repo's `.gitignore` before allocation."),
            Scaffold(" "),
            Operative("A sandboxed Codex seat receives a prepared registered path and never runs `git worktree add` or creates a nested clone under that root."),
            Scaffold(" "),
            Operative("The managed root is allowlisted, predictable, and removable with `git worktree remove`; it is not an arbitrary `/tmp/intent-review-...` path.")),
        Fragment(
            S9,
            Operative("- **approval policy** — `approval_policy=never` / `danger-full-access` is NOT a substitute for safe routing or cleanup design."),
            Scaffold(" "),
            Operative("Keep least-privilege approvals as the default; route `.git` work to the non-sandboxed host-state role and use an explicitly declared ordinary temporary checkout when required, rather than suppressing a destructive `rm -rf` prompt.")),
        Fragment(S9, Operative("- Host-state role: add `.intent-cli/worktrees/` (or the configured `[project] worktree_root`) to the target repo `.gitignore`, then create each registered worktree with `git worktree add .intent-cli/worktrees/<role>-<unit> <branch>`.")),
        Fragment(S9, Operative("- Sandboxed Codex seat: use the prepared path carried by the delegation; do not run `git worktree add`, write `.git`, or create a nested clone under the managed root.")),
        Fragment(S9, Operative("- Sandbox-safe fallback: only when the delegation explicitly supplies it, use a unit-scoped ordinary checkout under the declared role-work-root (for example `/private/tmp/<role>-<unit>`); this is not a registered worktree and the host-state role owns cleanup.")),
        Fragment(S9, Operative("- One worktree per role/unit; do not reuse a dirty worktree across units.")),
        Fragment(S9, Operative("- Remove a worktree only with `git worktree remove`; registered cleanup belongs to the non-sandboxed host-state role.")),
        Fragment(S9, Operative("- The non-sandboxed host-state role removes a registered worktree only with `git worktree remove` (it refuses a dirty worktree) — never raw `rm -rf`.")),
        Fragment(S9, Operative("- A sandboxed Codex seat does not remove registered worktrees or mutate `.git`; it reports cleanup/blocker state to the host-state role.")),
        Fragment(S9, Operative("- For an explicitly delegated ordinary temporary checkout under the role-work-root, the host-state role performs the approved cleanup after verifying the unit path; the seat never improvises a raw delete.")),
        ...
        Fragment(S9, Operative("- A sandboxed seat is being asked to register a worktree, write `.git`, or create a nested clone under the managed root — route it to the host-state role.")),
        ...
        Fragment(
            ...
            Scaffold(" "),
            Operative("A declared ordinary role-work-root checkout may be used only when the delegation names it and the host-state role owns cleanup;"),
            Scaffold(" "),
            Operative("reaching for an unmanaged path is a signal to STOP and route the repair, not to ask for `rm -rf` approval.")),
        Fragment(
            S10,
    ...
    [
        ...
        Fragment("design_workspace_supervision", Operative("intent-cli notify supervise install --domain __DOMAIN__ --team __TEAM__ --repo __OWNER__/__REPO__ --owner-role orchestration --bound 900 --interval 300 --startup-bound 120 --write --format json")),
        Fragment("design_workspace_supervision", Operative("intent-cli notify supervise install --domain __DOMAIN__ --team <team> --repo __OWNER__/__REPO__ --owner-role orchestration --bound 900 --interval 300 --startup-bound 120 --write --format json")),
        ...
        Fragment("design_workspace_supervision", Operative("G781 startup proof: `--startup-bound <seconds>` defaults to 120s, reads at most once per second, and must observe a writer-bearing first cycle before a write reports success")),
        ...
        Fragment("design_workspace_supervision", Operative("intent-cli notify supervise install --domain __DOMAIN__ --team __TEAM__ --repo __OWNER__/__REPO__ --owner-role orchestration --bound 900 --interval 300 --startup-bound 120 --platform macos --write --format json")),
        Fragment("design_workspace_supervision", Operative("intent-cli notify supervise install --domain __DOMAIN__ --team <team> --repo __OWNER__/__REPO__ --owner-role orchestration --bound 900 --interval 300 --startup-bound 120 --platform macos --write --format json")),
        ...
        Fragment("design_workspace_supervision", Operative("G781 first-cycle proof failure is named `first-cycle-proof-failed`: `no-post-install-process` names registration without a missing log path, while `post-install-process-wrote-no-cycle` names only existing logs; `install --verify` proves a late start")),
        ...
        Fragment(
            "design_handoff",
            Operative("Routine delegation (host-state publish routing, delegate, CI wait, review, closeout) stays orchestrator↔receivers/host-state role and does NOT go to design."),
            Scaffold(" "),
            Operative("Return to DESIGN only for human decisions — product/design clarification, release/credentials/security, destructive actions, or an unresolved blocker — using the structured escalation message (reason / current_state / evidence / decision_needed).")),
        Fragment(
            "design_handoff",
        ...
        Fragment(
            "worktree_management",
            Operative("The non-sandboxed host-state role allocates registered worktrees under a repo/workspace-scoped managed root — the `[project] worktree_root` (default `.intent-cli/worktrees/`) — and the exact managed root MUST be listed in the target repo's `.gitignore` before allocation."),
            Scaffold(" "),
            Operative("A sandboxed Codex seat receives a prepared registered path and never runs `git worktree add` or creates a nested clone under that root."),
            Scaffold(" "),
            Operative("The managed root is allowlisted, predictable, and removable with `git worktree remove`; it is not an arbitrary `/tmp/intent-review-...` path.")),
        Fragment(
            "worktree_management",
            Operative("`approval_policy=never` / `danger-full-access` is NOT a substitute for safe routing or cleanup design."),
            Scaffold(" "),
            Operative("Keep least-privilege approvals as the default; route `.git` work to the non-sandboxed host-state role and use an explicitly declared ordinary temporary checkout when required, rather than suppressing a destructive `rm -rf` prompt.")),
        Fragment("worktree_management", Operative("Host-state role: add `.intent-cli/worktrees/` (or the configured `[project] worktree_root`) to the target repo `.gitignore`, then create each registered worktree with `git worktree add .intent-cli/worktrees/<role>-<unit> <branch>`.")),
        Fragment("worktree_management", Operative("Sandboxed Codex seat: use the prepared path carried by the delegation; do not run `git worktree add`, write `.git`, or create a nested clone under the managed root.")),
        Fragment("worktree_management", Operative("Sandbox-safe fallback: only when the delegation explicitly supplies it, use a unit-scoped ordinary checkout under the declared role-work-root (for example `/private/tmp/<role>-<unit>`); this is not a registered worktree and the host-state role owns cleanup.")),
        Fragment("worktree_management", Operative("One worktree per role/unit; do not reuse a dirty worktree across units.")),
        Fragment("worktree_management", Operative("Remove a worktree only with `git worktree remove`; registered cleanup belongs to the non-sandboxed host-state role.")),
        Fragment("worktree_management", Operative("The non-sandboxed host-state role removes a registered worktree only with `git worktree remove` (it refuses a dirty worktree) — never raw `rm -rf`.")),
        Fragment("worktree_management", Operative("A sandboxed Codex seat does not remove registered worktrees or mutate `.git`; it reports cleanup/blocker state to the host-state role.")),
        Fragment("worktree_management", Operative("For an explicitly delegated ordinary temporary checkout under the role-work-root, the host-state role performs the approved cleanup after verifying the unit path; the seat never improvises a raw delete.")),
        ...
        Fragment("worktree_management", Operative("A sandboxed seat is being asked to register a worktree, write `.git`, or create a nested clone under the managed root — route it to the host-state role.")),
        ...
        Fragment("next_slice_publication", Operative("Authorized host-state role executes: intent-cli issue publish-flow <execution-unit> --repo __OWNER__/__REPO__ --write --format json")),
        Fragment("next_slice_publication", Operative("Authorized host-state role executes: intent-cli automation issue-publish --write --format json")),
        Fragment(
            "next_slice_publication",
            Operative("The orchestrator routes and verifies; a sandboxed Codex seat never executes these write-bearing commands itself."),
            Scaffold(" "),
            Operative("Never raw `gh issue create` or `gh ... --add-label`; publication and the `intent-target` label go through the canonical intent-cli surfaces only.")),
        Fragment("next_slice_publication", Operative("Confirm via intent-cli / GitHub (not chat) that the host-state role's result is terminal, the issue exists with the expected execution-unit body, and the `intent-target` label is present.")),
        Fragment("next_slice_publication", Operative("Confirm the durable workflow state (queue-state / linkage / label) reflects the host-state role's publish through intent-cli surfaces.")),
        Fragment(
            "next_slice_publication",
            Transport("Immediately after verification, in THIS SAME WAKE, delegate implementation over agmsg (G524) — do not stop after publishing and wait for a future wake to send the delegation."),
            Scaffold(" "),
